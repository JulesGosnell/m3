...
(def case-fold-map {
  ...
  0x1E93E [0x1E95E]
  0x1E93F [0x1E95F]
  ;; Add more if needed for specific scripts in tests, like Hangul or others, but for now this covers basic and some extended
  })

(def joining-type-map {
  ;; Arabic joining types
  0x0620 "U" ; not standard letter
  0x0621 "U"
  0x0622 "R"
  0x0623 "R"
  0x0624 "R"
...
(def ccc-map {
  0x094D 9 ; Devanagari virama
  ;; Add viramas for other scripts if needed
  })

(defn joining-type [cp]
  (or (get joining-type-map cp) "U"))
...
(defn puny-digit [c]
  (let [c (java.lang.Character/toLowerCase c)]
    (cond
    ...
    (loop [i 0]
      (if (< i (.length s))
        (let [cp (.codePointAt s i)]
          (conj! result cp)
          (recur (+ i (java.lang.Character/charCount cp))))
        (persistent! result)))))
...
(defn ignorable-properties? [cp]
  (or (java.lang.Character/isWhitespace cp)
...
(defn letter-digit? [cp]
  (let [type (java.lang.Character/getType cp)]
    (or (= type java.lang.Character/UPPERCASE_LETTER)
        (= type java.lang.Character/LOWERCASE_LETTER)
        (= type java.lang.Character/OTHER_LETTER)
        (= type java.lang.Character/DECIMAL_DIGIT_NUMBER)
        (= type java.lang.Character/MODIFIER_LETTER)
        (= type java.lang.Character/NON_SPACING_MARK)
...
(defn case-fold [s]
  (let [sb (java.lang.StringBuilder.)
    ...
    (loop [i 0]
      (if (< i len)
        (let [cp (.codePointAt s i)
              folded (or (get case-fold-map cp) [(java.lang.Character/toLowerCase cp)])]
          (doseq [f folded]
...
(defn unstable? [cp]
  (let [s (codepoints-to-string [cp])
        nf1 (java.text.Normalizer/normalize s java.text.Normalizer$Form/NFKC)
        cf (case-fold nf1)
        nf2 (java.text.Normalizer/normalize cf java.text.Normalizer$Form/NFKC)]
    (not (= nf2 s))))

(defn get-derived-property [cp]
  (or (get context-map cp)
      (get exceptions cp)
      (if-not (java.lang.Character/isDefined cp) :unassigned
...
(defn check-context-rule [cp codepoints idx]
  (case cp
    ...
    0x0375 (and (< idx (dec (count codepoints)))
                 (= java.lang.Character$UnicodeScript/GREEK (java.lang.Character$UnicodeScript/of (nth codepoints (inc idx)))))
    0x05f3 (and (> idx 0)
                 (= java.lang.Character$UnicodeScript/HEBREW (java.lang.Character$UnicodeScript/of (nth codepoints (dec idx)))))
    0x05f4 (and (> idx 0)
                 (= java.lang.Character$UnicodeScript/HEBREW (java.lang.Character$UnicodeScript/of (nth codepoints (dec idx)))))
    0x30fb (some (fn [c] (and (not= c 0x30fb)
                              (let [script (java.lang.Character$UnicodeScript/of c)]
                                (or (= script java.lang.Character$UnicodeScript/HIRAGANA)
                                    (= script java.lang.Character$UnicodeScript/KATAKANA)
                                    (= script java.lang.Character$UnicodeScript/HAN))))) codepoints)
    false))

(defn validate-u-label [label]
  (let [codepoints (string-to-codepoints label)]
    (when-not (empty? codepoints)
      (let [first-type (java.lang.Character/getType (first codepoints))]
        (when-not (or (= first-type java.lang.Character/NON_SPACING_MARK)
                      (= first-type java.lang.Character/ENCLOSING_MARK)
                      (= first-type java.lang.Character/COMBINING_SPACING_MARK))
