...
(def draft->vocab-and-property-and-semantics
  {"draft3"
   [[:validation "type" check-property-type]
    [:validation "const" check-property-const]
    [:validation "minLength" check-property-minLength]
    [:validation "maxLength" check-property-maxLength]
    [:validation "multipleOf" check-property-multipleOf]
    [:validation "format" check-property-format]
    [:validation "enum" check-property-enum]
    [:validation "pattern" check-property-pattern]
    [:core "$ref" check-property-$ref]
    [:core "$schema" check-property-$schema]
    [:core "$id" check-property-$id]
    [:core "id" check-property-id]
    [:core "$anchor" check-property-$anchor]
    [:core "$recursiveRef" check-property-$recursiveRef]
    [:core "$recursiveAnchor" check-property-$recursiveAnchor]
    [:core "$dynamicRef" check-property-$dynamicRef]
    [:core "$dynamicAnchor" check-property-$dynamicAnchor]
    [:core "$vocabulary" check-property-$vocabulary]
    [:meta-data "title" check-property-title]
    [:meta-data "description" check-property-description]
    [:meta-data "default" check-property-default]
    [:meta-data "examples" check-property-examples]
    [:meta-data "deprecated" check-property-deprecated]
    [:meta-data "readOnly" check-property-readOnly]
    [:meta-data "writeOnly" check-property-writeOnly]
    [:core "$comment" check-property-$comment]
    [:core "definitions" check-property-definitions]
    [:core "$defs" check-property-$defs]
    [:applicator "propertyNames" check-property-propertyNames]
    [:applicator "propertyDependencies" check-property-propertyDependencies]
    [:validation "minProperties" check-property-minProperties]
    [:validation "maxProperties" check-property-maxProperties]
    [:applicator "not" check-property-not]
    [:applicator "anyOf" check-property-anyOf]
    ...
    [:applicator "properties" check-property-properties]
    [:applicator "patternProperties" check-property-patternProperties]
   ...
   [[:validation "type" check-property-type]
    ...
    [:validation "maxLength" check-property-maxLength]
    [:validation "multipleOf" check-property-multipleOf]
    [:format-assertion "format" check-property-format]
    [:validation "enum" check-property-enum]
    [:validation "pattern" check-property-pattern]
    [:core "$ref" check-property-$ref]
    [:core "$schema" check-property-$schema]
